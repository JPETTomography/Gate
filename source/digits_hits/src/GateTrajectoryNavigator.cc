...
std::vector<G4int> GateTrajectoryNavigator::GetPhotonIndices() {
  ...
  for (G4int iTrj=0; iTrj<n_trajectories; iTrj++) {
    ...
    if (m_positronTrackID != 0) {
      ...
      if ((trj->GetParentID() == m_positronTrackID) && (trj->GetPDGEncoding() == 22) ) { // 22 == Gamma
...
void GateTrajectoryNavigator::FillPhotonIDsForThreePhotons(std::vector<G4int>& photonIndices, bool& only2gamma) {
...
G4int GateTrajectoryNavigator::FindPhotonID(G4int trackID)
{
  ...
  if (!m_trajectoryContainer) {
    ...
    if (m_photonIDVec.size() == 0) {
      ...
      if (photonID == rootID) {
        if (nVerboseLevel > 2) {
          G4cout << "GateTrajectoryNavigator::FindPhotonID : trackID: " << trackID << " photonID = " << rootID << G4endl;
        }
      }
