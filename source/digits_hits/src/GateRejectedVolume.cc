...
GateRejectedVolume::GateRejectedVolume(GatePulseProcessorChain* itsChain, const G4String& itsName, G4String rejectedVolume)
    : GateVPulseProcessor(itsChain, itsName), m_rejectedVolume(rejectedVolume)
{
  m_messenger = new GateRejectedVolumeMessenger(this);
}

GateRejectedVolume::~GateRejectedVolume() { delete m_messenger; }
...
void GateRejectedVolume::DescribeMyself(size_t indent) { G4cout << GateTools::Indent(indent) << "Rejected volume: " << m_rejectedVolume << Gateendl; }
