...
// --------------------------------------------------------------------
GateJPetActor::GateJPetActor(G4String name, G4int depth):
  GateVActor(name, depth) {
  GateDebugMessageInc("Actor", 4, "GateJPetActor() -- begin\n");
  ...
  pIAEAheader = 0;
  mFileSize = 0;
  GateDebugMessageDec("Actor", 4, "GateJPetActor() -- end\n");
...
void GateJPetActor::Construct() {
  ...
  if (mFileType == "rootFile") {
    ...
    pListeVar = new TTree("JPet", "Phase space tree");
...
void GateJPetActor::UserSteppingAction(const GateVVolume *, const G4Step *step) {
  ...
  if (mStoreOutPart) {
     ...
     * There is a rare bug when using the JPetActor to store outgoing particles and very long cuts on particles (nongammas).
  ...
  */
  ...
  //Solution, use PDGcode instead of ParticleName. However, GateJPetSource uses Particlename char[64] while GateJPetActor stores Char_t[256].
